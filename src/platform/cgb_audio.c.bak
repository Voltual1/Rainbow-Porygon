...
void cgb_set_wavram(void){
    volatile u8 *waveRam = (volatile u8 *)REG_ADDR_WAVE_RAM0;
    for(u8 wavi = 0; wavi < 0x10; wavi++){
        gb.WAVRAM[(wavi << 1)] = ((waveRam[wavi] & 0xF0) >> 4) / 7.5f - 1.0f;
        gb.WAVRAM[(wavi << 1) + 1] = ((waveRam[wavi] & 0x0F)) / 7.5f - 1.0f;
    }
}
