 ...
 */
package me.voltual.rp.core.utils.extension.text
...
fun Long.formatSize(): String {
  val (size, index) =
    generateSequence(Pair(this.toFloat(), 0)) { (size, index) ->
        if (size >= 1024f) Pair(size / 1024f, index + 1) else null
      }
      .take(sizeFormats.size)
      .last()
...
fun getIsoDateOfMonthsAgo(months: Int): String =
  Clock.System.now()
    .toLocalDateTime(TimeZone.currentSystemDefault())
    .date
    .minus(DatePeriod(months = months, days = 1))
    .toString()
